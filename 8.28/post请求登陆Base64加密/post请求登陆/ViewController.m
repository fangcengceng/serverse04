...
- (void)viewDidLoad {
     ...
     */
    //从系统偏好设置中读取数据,  保存的时机是在登陆成功之后，再将密码和用户名保存
    [self readinfos];
    
    /*加密时机
     1、向网络传输数据是作为请求体的时候需要加密
     2、保存到系统偏好设置中是需要加密，因为不能已明码的方式显示密码
     3、从系统偏好中读取信息时需要加密
    */
...
#pragma 将隐私信息密码存储到系统偏好中；保存的时机是登陆成功之后；
-(void)saveinfo{
    //用户名不需要加密，因为不属于隐私信息
    [[NSUserDefaults standardUserDefaults] setObject:self.usertextField.text forKey:KUSERNAME];
    //用户密码属于隐私信息，需要进行加密.base64编码的原理是将二进制数据编译成base64编码(编译前为二进制数  ,编译后是字符串)；
    NSData *data = [self.passworlField.text dataUsingEncoding:NSUTF8StringEncoding];
   NSString *passworld = [data base64EncodedStringWithOptions:0];
    [[NSUserDefaults standardUserDefaults] setObject:passworld forKey:KPASSWORD];
//    [[NSUserDefaults standardUserDefaults] setObject:self.passworlField.text forKey:KPASSWORD];//key值是自己任意起的，为了方便最好用宏
//    NSLog(@"已经被保存到系统偏好设置之后 ，保密前可以打印出来密码，不安全== =%@",self.passworlField.text);
    
}
#pragma 从系统偏好中将密码读取出来
-(void)readinfos{
    self.usertextField.text =[[NSUserDefaults standardUserDefaults] objectForKey:KUSERNAME];
    //在读取秘密之前先解密,然后在给self.passfield.text 赋值
    NSString *str = [[NSUserDefaults  standardUserDefaults] objectForKey:KPASSWORD];
    //转为二进制数
    if( str.length == 0){
        return;
    }
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:0];
   self.passworlField.text = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    NSLog(@"解码后==%@",self.passworlField.text);
}
