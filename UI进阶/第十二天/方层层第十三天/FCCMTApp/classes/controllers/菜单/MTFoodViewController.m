...
@interface MTFoodViewController ()<UITableViewDelegate,UITableViewDataSource,MTFoodCellDelegate,MTCarVIewsDelegate>
...
@implementation MTFoodViewController{
    ...
    NSMutableArray<MTFoodsModel*> *_carList;
...
-(void)bottomCarView:(MTCarVIews *)bottomCarView needsDisplay:(NSMutableArray<MTFoodsModel *> *)selectedModel{
    ...
    MTBottomModalController * modalController = [[MTBottomModalController alloc] init];
...
#warning mark 开始动画的时候接受数据。
    if(![_carList containsObject:foodModel]){
        [_carList addObject:foodModel];
    }
...
-(void)foodCell:(MTFoodCell *)foodCell didFinishDecreaseFood:(MTFoodsModel *)foodModel{
    if(foodModel.orderCount == 0){
        [_carList removeObject:foodModel];
    }
    _carView.shoplist = _carList;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MTFoodsModel *nextModel =  _catagoryList[indexPath.section].spus[indexPath.row];
    
//    MTDetaliViewController *detailController = [[MTDetaliViewController alloc] init];
//    
//    detailController.selectModel = _catagoryList[indexPath.section].spus[indexPath.row];
//    
//    [self.navigationController pushViewController:detailController animated:NO];
  
    
    MTNormalController * normalController = [[MTNormalController alloc] init];
    normalController.catagoryModel = _catagoryList;
    normalController.path = indexPath;
    [self.navigationController pushViewController:normalController animated:NO];
...
- (void)setupUI{
   ...
   //系统提供了数据源方法；
    ...
    MTCarVIews *carView = [[NSBundle mainBundle] loadNibNamed:@"MTCarVIews" owner:nil options:nil].lastObject;
