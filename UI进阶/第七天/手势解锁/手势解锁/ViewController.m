...
- (void)viewDidLoad {
    ...
    [_demoView addTarget:self action:@selector(demoViewValueChanged:) forControlEvents:UIControlEventValueChanged];
    
}
-(void)demoViewValueChanged:(HMDemoView *) demoView{
    ...
    if(savePass.length==0){
        [defaults setObject:demoView.psw forKey:@"HMDemoWord"];
    ...
    if([savePass isEqualToString:demoView.psw]){
